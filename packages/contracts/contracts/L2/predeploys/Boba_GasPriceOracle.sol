...
contract Boba_GasPriceOracle {
    ...
    // 출금 시 fee를 보관하는 주소. l2geth 내에서 동적으로 초기화됨
    ...
    //보바를 수수료 토큰으로 사용하고자 하는 지갑 주소를 기록합니다.
    // key 타입 address => value 타입 bool
     ...
     * BOBA를 요금 토큰으로 사용하려는 사용자 추가
     */
    function useBobaAsFeeToken() public {
        // 컨트랙트를 호출한 account가 EOA인지 확인. EOA가 아니라면 종료
    ...
    //     // totalCost = receivedETHAmount * marketPriceRatio + metaTransactionFee
