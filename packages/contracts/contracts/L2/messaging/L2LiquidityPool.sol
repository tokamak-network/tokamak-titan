...
import "../predeploys/OVM_GasPriceOracle.sol";
import "./L2StandardBridge.sol";
import "../../L1/messaging/IL1LiquidityPool.sol";
...
contract L2LiquidityPool is CrossDomainEnabled, ReentrancyGuardUpgradeable, PausableUpgradeable {
     ...
     */
    function updateUserRewardPerShare(address _tokenAddress) public {
        PoolInfo storage pool = poolInfo[_tokenAddress];
        if (pool.lastAccUserReward < pool.accUserReward) {
            uint256 accUserRewardDiff = pool.accUserReward - pool.lastAccUserReward;
            if (pool.userDepositAmount != 0) {
                pool.accUserRewardPerShare =
                    pool.accUserRewardPerShare +
                    ((accUserRewardDiff * 1e12) / pool.userDepositAmount);
            }
     ...
     */
    function addLiquidity(uint256 _amount, address _tokenAddress)
        external
        payable
        nonReentrant
        whenNotPaused
    {
        ...
        pool.userDepositAmount = pool.userDepositAmount + _amount;
        ...
        whenNotPaused
    {
        ...
        uint256 ownerRewardFee = (_amount * ownerRewardFeeRate) / 1000;
