...
import ICrossDomainMessenger from '../../../../artifacts/contracts/libraries/bridge/ICrossDomainMessenger.sol/ICrossDomainMessenger.json'
...
describe('L1ERC721Bridge', () => {
  ...
  beforeEach(async () => {
    ...
    // set to _balances (aliceAddress: 5)
    ...
    it('initializes correctly', async () => {
      it('reverts if cross domain messenger is address(0)', async () => {
        await expect(
          Factory__L1ERC721Bridge.deploy(
            constants.AddressZero, // messenger
            DUMMY_L2_BRIDGE_ADDRESS
          )
      ...
      it('reverts if other bridge is address(0)', async () => {
        await expect(
          Factory__L1ERC721Bridge.deploy(
            Fake__L1CrossDomainMessenger.address,
            constants.AddressZero // other bridge
          )
  ...
  describe('ERC721 deposits', () => {
    ...
    it('bridgeERC721() reverts if remote token is address(0)', async () => {
      ...
      await expect(
        L1ERC721Bridge.connect(alice).bridgeERC721(
          L1ERC721.address,
          constants.AddressZero, // remote token
          tokenId,
          FINALIZATION_GAS,
          NON_NULL_BYTES32
        )
    ...
    it('bridgeERC721() escrows the deposit and sends the correct deposit message', async () => {
      ...
      // deposit: alice send 1 balance to L1ERC721Bridge
      ...
      expect(depositCallToMessenger.args[1]).to.equal(
        IL2ERC721Bridge.encodeFunctionData('finalizeBridgeERC721', [
          DUMMY_L2_ERC721_ADDRESS, // local token address
          L1ERC721.address, // remote token address
          aliceAddress, // from
          aliceAddress, // to
          tokenId,
          NON_NULL_BYTES32,
    ...
    it('bridgeERC721To() escrows the deposited NFT and sends the correct deposit message', async () => {
      ...
      await expect(
        L1ERC721Bridge.connect(alice).bridgeERC721To(
          L1ERC721.address,
          DUMMY_L2_ERC721_ADDRESS,
          bobsAddress, // receiver (when from is not equal to)
          tokenId,
          FINALIZATION_GAS,
          NON_NULL_BYTES32
        )
    ...
    describe('Handling ERC721.transferFrom() failures that revert', () => {
      // msg.sender is bob (incorrect owner)
      ...
      it('bridgeERC721To(): will revert if the L1 ERC721 is zero address', async () => {
        await expect(
          L1ERC721Bridge.connect(alice).bridgeERC721To(
            constants.AddressZero, // L1 ERC721 is zero address
            DUMMY_L2_ERC721_ADDRESS,
            bobsAddress,
            tokenId,
            FINALIZATION_GAS,
      ...
      it('bridgeERC721To(): will revert if the L1 ERC721 has no code', async () => {
        await expect(
          L1ERC721Bridge.connect(alice).bridgeERC721To(
            bobsAddress, // L1 ERC721 is a non-contract account
            DUMMY_L2_ERC721_ADDRESS,
            bobsAddress,
            tokenId,
            FINALIZATION_GAS,
